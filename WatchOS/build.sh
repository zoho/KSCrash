...
xcodebuild -project "${PROJECT_NAME}.xcodeproj" archive -scheme "${SCHEME}" -destination="generic/platform=watchOS" -archivePath archive/watchos.xcarchive -sdk watchos SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

xcodebuild -project "${PROJECT_NAME}.xcodeproj" archive -scheme "${SCHEME}" -destination="generic/platform=watchOS Simulator" -archivePath archive/watchsimulator.xcarchive -sdk watchsimulator SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

xcodebuild -create-xcframework -framework "Archive/watchos.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -framework "Archive/watchsimulator.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -output  "output/${FRAMEWORK_NAME}.xcframework"
