...
PROJECT_NAME="KSCrash-iOS"
FRAMEWORK_NAME="KSCrash"
SCHEME="KSCrash_static"
...
xcodebuild -project "${PROJECT_NAME}.xcodeproj" archive -scheme "${SCHEME}" -destination="generic/platform=iOS" -archivePath archive/iphoneos.xcarchive -sdk iphoneos SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

xcodebuild -project "${PROJECT_NAME}.xcodeproj" archive -scheme "${SCHEME}" -destination="generic/platform=iOS Simulator" -archivePath archive/iphonesimulator.xcarchive -sdk iphonesimulator SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

# xcodebuild -create-xcframework -framework "Archive/iphoneos.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -framework "Archive/iphonesimulator.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -output  "output/${FRAMEWORK_NAME}.xcframework"

xcodebuild -project "${PROJECT_NAME}.xcodeproj" archive -scheme "${SCHEME}" -destination="platform=macOS,arch=x86_64,variant=Mac Catalyst" -archivePath archive/mac-catalyst.xcarchive -sdk macosx SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

xcodebuild -create-xcframework -framework "Archive/iphoneos.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -framework "Archive/iphonesimulator.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -framework "Archive/mac-catalyst.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -output  "output/${FRAMEWORK_NAME}.xcframework"
