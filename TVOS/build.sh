...
xcodebuild -project "${PROJECT_NAME}.xcodeproj" archive -scheme "${SCHEME}" -destination="generic/platform=tvOS" -archivePath archive/tvos.xcarchive -sdk appletvos SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

xcodebuild -project "${PROJECT_NAME}.xcodeproj" archive -scheme "${SCHEME}" -destination="generic/platform=tvOS Simulator" -archivePath archive/tvsimulator.xcarchive -sdk appletvsimulator SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

xcodebuild -create-xcframework -framework "Archive/tvos.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -framework "Archive/tvsimulator.xcarchive/Products/Library/Frameworks/${FRAMEWORK_NAME}.framework" -output  "output/${FRAMEWORK_NAME}.xcframework"
